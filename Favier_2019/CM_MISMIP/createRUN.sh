...
MELT_PARAM=$4          ## Melt parameterization. Choice in:
...
LAZER_TYPE=6           ##few choices for Lazeroms law
                       ##1 is simple law (using the slope between the draft point and the grounding line point)
                       ##2 is Lazeroms TC law
                       ##3 is AppendixB law
                       ##4 is Jenkins law (using the slope between the draft point and the grounding line point)
                       ##5 is simple law with local gradient
                       ##6 is Jenkins law with local gradient
...
WORKDIR_ELMER=${SCRATCHDIR}/ELMERICE/PARAMS
STORE_ELMER=${STOREDIR}/ELMERICE/PARAMS
...
then
	...
	echo "1st: Case name "
        echo "2nd: melting coef "
        echo "third: 'EXP??' "
        echo "fourth: 'param' (bg, pdc, pdcstar, lazer) " 
        exit 1
fi
...
cat Scripts/scriptIce1rExecute.sh | sed -e "s#<run>#$1#g" \
                 ...
                 -e "s#<Boxes>#$BOXES#g" \
                 -e "s#<LazerType>#$LAZER_TYPE#g" \
                 -e "s#<EXP>#$FORCING_EXP_ID#g" \
                 -e "s#<MeshNamePath>#$WORKDIR_ELMER/runs/$1#g" \
                 -e "s#<STORE_ELMER>#$STORE_ELMER#g" > $ELMER_WORK_PATH/scriptIce1rExecute.sh
