...
INTEGER :: ngl, nglloc, ngltot, cpt, nn, debilos, dir, nbdir, cpttmp, jj, hwidth, kkstart, halfw, halfpt, kk1, &
&          cptglob
...
CASE('bg')
   ...
   DO node=1,Nmax 
     ...
     if ( GM(GMPerm(node)) .lt. mskcrit .and. zzz .ge. epsz ) then
        ! Temperature and salinity at the depth of the ice draft :
        if ( zzz .le. z1 ) then
          T0 = Tsrf
          S0 = Ssrf + zzz * S_stratif
        elseif ( zzz .ge. z2 ) then
          T0 = Tbot
          S0 = Sbot + zzz * S_stratif
        else
          T0 = (Tbot-Tsrf)*zzz/(z2-z1) + Tsrf - (Tbot-Tsrf)*z1/(z2-z1)
          S0 = (Sbot-Ssrf)*zzz/(z2-z1) + Ssrf - (Sbot-Ssrf)*z1/(z2-z1) + zzz*S_stratif
        endif
...
CASE('lazer')
   ...
   if (LazerType .eq. 4) then
     ...
     !do ii=1,SIZE(xgltot2)
     !  print *, 'xgltot', xgltot2(ii)
     !  print *, 'ygltot', ygltot2(ii)
